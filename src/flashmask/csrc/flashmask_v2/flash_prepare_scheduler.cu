...
// fallback, in case that runtime caller does not allocate valid GPU mem for semaphore
static __device__ int semaphore_storage_fwd[1];
static __device__ int semaphore_storage_bwd[1];
// fallback, if overlap is good to go, we use int buffer allocated by runtime
