...
#define FLASHMASK_CHECK(__cond, message)                    \
      do {                                               \
...
#define CHECK_DEVICE(x) FLASHMASK_CHECK(x.is_cuda(), #x " must be on CUDA")
#define CHECK_SHAPE(x, ...) FLASHMASK_CHECK(x.sizes() == torch::IntArrayRef({__VA_ARGS__}), #x " must have shape (" #__VA_ARGS__ ")")
#define CHECK_CONTIGUOUS(x) FLASHMASK_CHECK(x.is_contiguous(), #x " must be contiguous")

void run_mha_fwd(Flash_fwd_params &params, cudaStream_t stream) {
    // HEADDIM_SWITCH(params.d, [&] {
    //     run_mha_fwd_<cutlass::half_t, kHeadSize>(params, stream);
    // });
    FLASHMASK_CHECK(params.num_splits >= 1, "num_splits should >= 1");
                    ...
                    SOFTCAP_SWITCH(params.softcap > 0.0, Has_softcap, [&] {
                        if (!params.is_e4m3) {
                            if (params.is_bf16) {
                                ...
                                #endif
                                #else
                                FLASHMASK_CHECK(false, "This flash attention build does not support FP16.");
                                #endif
                            }
                        } else {
...
void run_mha_fwd_combine(Flash_fwd_params &params, cudaStream_t stream, bool enable_pdl=false) {
    ...
    #else
    FLASHMASK_CHECK(false, "This flash attention build does not support combine kernels.");
    #endif
}
            ...
            BOOL_SWITCH(params.is_causal, Is_causal, [&] {
                BOOL_SWITCH(params.deterministic, Deterministic, [&] {
                    if (!params.is_bf16) {
                        ...
                        #endif
                        FLASHMASK_CHECK(false, "This flash attention build does not support ");
                    }
                });
            });
        });
...
void flashmaskv2_destroy_fwd_params_handle(Flash_fwd_params* params_handle) {
    FLASHMASK_CHECK(params_handle, "params_handle is nullptr");
    free(params_handle);
}
