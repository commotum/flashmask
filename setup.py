...
def build_mode() -> str | None:
    ...
    if build_stub and build_experimental:
        raise RuntimeError(
            f"Set only one of {BUILD_STUB_ENV}=1 or {BUILD_EXPERIMENTAL_ENV}=1."
        )
...
def load_torch_extension(build_kind: str):
    try:
        ...
        env_name = BUILD_EXPERIMENTAL_ENV if build_kind == "experimental" else BUILD_STUB_ENV
        raise RuntimeError(
